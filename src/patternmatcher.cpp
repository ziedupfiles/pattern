        ...
        if (allElements.size() > 0) {
            for (auto ele = allElements.begin(); ele != allElements.end(); ele++) {
                if ((patternElement.linkPreElement == "or") or ((patternElement.linkPreElement == "and " or patternElement.linkPreElement == "sequence ") and (list2).size() > 0)) {
                    ...
                    eventElement tempele = eventElement(EventValues::EVENT_DEFAULT, preElement->timeStamp,0);
...
std::vector<eventElement> PatternMatcher::returnAll(std::vector<eventElement> PipeLine, EventValues patternElementVal, int duration , float timeStamp , int direction) {
    std::vector<eventElement> newElementList;
...
std::vector<eventElement> PatternMatcher::returnAllNext(std::vector<eventElement> PipeLine, PatternEvent patternElement, int duration , float timeStamp , int direction ) {
    std::vector<eventElement> newElementList;
